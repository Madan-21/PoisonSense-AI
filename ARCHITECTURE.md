...
│  │  │ (classify,   │──│ (decision    │──│ (Groq/OpenAI/Ollama)   │  │  │
│  │  │  refuse,     │  │  routing,    │  │                        │  │  │
│  │  │  escalate)   │  │  generate)   │  └────────────────────────┘  │  │
│  │  └─────────────┘  └──────┬───────┘                               │  │
│  │                          │                                        │  │
...
### Embedding Model
- **Default**: `sentence-transformers/all-MiniLM-L6-v2` (local, free, fast)
- **Alternative**: OpenAI `text-embedding-3-small` (set `EMBEDDING_PROVIDER=openai`)
- **Pluggable**: Change via environment variable
...
- **Embeddings**: sentence-transformers (local) or OpenAI (pluggable)
- **LLM**: Groq (default) / OpenAI / Ollama (pluggable)
