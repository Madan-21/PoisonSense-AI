...
# Lazy imports — RAG modules are loaded on first use
# so the serverless function cold-starts faster.
...
async def ingest_pdfs(
    ...
    Files are saved to a temporary location, ingested, then cleaned up.
    """
    import tempfile
    r = rag()
    results = []
    for file in files:
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(suffix=".pdf", delete=False) as tmp:
                tmp.write(content)
                tmp_path = tmp.name
            result = r["ingest_pdf"](tmp_path, collection_name=collection, doc_title=file.filename)
