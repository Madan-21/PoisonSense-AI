...
from app.models.otp import PendingSignup
...
def _get_pending(email: str, db: Session) -> PendingSignup | None:
    """Look up a pending signup row."""
    return db.query(PendingSignup).filter(PendingSignup.email == email).first()


def _set_pending(email: str, data: dict, db: Session):
    """Upsert a PendingSignup row."""
    existing = _get_pending(email, db)
    if existing:
        for k, v in data.items():
            if hasattr(existing, k):
                setattr(existing, k, v)
    else:
        row = PendingSignup(email=email, **data)
        db.add(row)
    db.commit()


def _del_pending(email: str, db: Session):
    """Delete a pending signup row."""
...
async def verify_email_otp(data: VerifyOTPRequest, db: Session = Depends(get_db)):
    ...
    pending = _get_pending(email, db)
    if not pending and not existing_user:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    if existing_user:
        ...
        role_str = (pending.role or "patient").lower()
        if role_str == "doctor":
        ...
        user = User(
            ...
            hospital_address=pending.hospital_address,
            is_active=True,
            is_verified=True,
...
async def upload_license(
    ...
    relative_path = f"{settings.STORAGE_BUCKET}/{object_key}"
